...
export default function Navbar() {
  ...
  return (
    <nav className="w-full top-0 left-0 z-50 fixed bg-white">
      <div className="max-w-screen-xl flex flex-wrap items-center justify-between mx-auto px-4 sm:px-6 md:px-20 py-4">
        <a href="/" className="flex items-center gap-3">
          <Image src="/logo.png" width={44} height={44} alt="Logo" />
          <span className="text-black text-xl font-bold">PORTFOLIO</span>
        </a>
        <button
        ...
        <div className={`${isOpen ? 'block' : 'hidden'} w-full md:block md:w-auto`} id="navbar-default">
          <ul className="font-medium flex flex-col p-4 md:p-0 mt-4 border rounded-lg md:flex-row md:items-center md:gap-8 md:mt-0 md:border-0 md:bg-white">
            {links.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
            ...
            <li className="mt-4 md:mt-0">
              <a
                ...
                className="flex items-center gap-2 px-5 py-3 bg-black text-white text-lg font-semibold rounded transition-opacity hover:opacity-90"
              >
                Resume
